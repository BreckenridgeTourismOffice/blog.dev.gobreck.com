...
if ( ! class_exists( 'CB_WP_Widget_Popular_Posts' ) ) {
    class CB_WP_Widget_Popular_Posts extends WP_Widget {
    	...
    	function widget($args, $instance) {
    		...
    		$cb_title = empty($instance['cb_title']) ? __('Popular Posts', 'cubell') : $instance['cb_title'];
                    ...
                    if ( function_exists( 'stats_get_csv' ) ) {
                        if ( $cb_filter_date == 'week' ) {
                            ...
                            if ( ( $cb_qry = get_transient( $cb_weekly_qry ) ) === false ) {
                                $cb_qry = stats_get_csv( 'postviews', 'days=8&limit=' . ( 3 + $cb_number ) );
                            ...
                            if ( ( $cb_qry = get_transient( $cb_monthly_qry ) ) === false ) {
                                $cb_qry = stats_get_csv( 'postviews', 'days=31&limit=' . ( 3 + $cb_number ) );
                            ...
                            if ( ( $cb_qry = get_transient( $cb_alltime_qry ) ) === false ) {
                                $cb_qry = stats_get_csv( 'postviews', 'days=-1&limit=' . ( 3 + $cb_number ) );
            ...
            if ( ( $cb_filter_by == 'cb-visits' ) && ( ! empty( $cb_qry ) ) &&  ( function_exists( 'stats_get_csv' ) ) ) {
                ...
                foreach ( $cb_qry as $cb_post ) {
                 ...
                 <li class="cb-article clearfix">
                    ...
                    <div class="cb-meta">
                        <h4<?php echo $cb_class; ?>><a href="<?php echo esc_url( $cb_post['post_permalink'] ); ?>"><?php echo esc_html( $cb_post['post_title'] ); ?></a></h4>
                        <?php echo cb_byline(false, $cb_post_id, true); ?>
                        <?php if ($cb_type == 'cb-big') { echo '<div class="cb-excerpt">'. cb_clean_excerpt(120).'</div>'; } ?>
                   </div>
                </li>
