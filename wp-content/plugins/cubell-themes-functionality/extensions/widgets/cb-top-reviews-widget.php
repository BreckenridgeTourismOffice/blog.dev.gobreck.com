...
if ( ! class_exists( 'CB_WP_Widget_top_reviews' ) ) {
    class CB_WP_Widget_top_reviews extends WP_Widget {
    	...
    	function widget($args, $instance) {
    		...
    		$cb_title = empty($instance['title']) ? __('Top Reviews', 'cubell') : $instance['title'];
...
?>
            <li <?php post_class('cb-article cb-relative cb-grid-entry cb-style-overlay cb-dark-bg clearfix' ); ?> >

                <div class="cb-mask"><?php cb_thumbnail('430', '270'); ?></div>
                <div class="cb-meta cb-article-meta">
                    <h4 class="cb-post-title h3"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h4>
                    <?php echo cb_byline(false); ?>
                </div>
                <div class="cb-countdown header-font"><?php echo $i; ?></div>
                <?php if ( $cb_type != 'cb-reader-score' ) { echo cb_review_ext_box($cb_post_id, $cb_category_color); } ?>
